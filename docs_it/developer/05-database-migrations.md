...
auth-service e catalog-service eseguono entrambi `alembic upgrade head` automaticamente prima di avviare uvicorn (configurato nel CMD del loro `Dockerfile`). Non è necessario eseguire le migrazioni manualmente in sviluppo locale o in produzione.
