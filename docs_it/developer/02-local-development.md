...
[`jinbocho-infrastructure-community-v1`](https://github.com/jinbocho/jinbocho-infrastructure-community-v1). Non
...
`jinbocho-infrastructure-community-v1` si aspetta che i repository dei servizi siano
clonati nella stessa directory padre, allo stesso livello:
...
Da `jinbocho-infrastructure-community-v1/`, copia il file `.env` di root e i template
...
`./scripts/dev.sh` avvia lo stack Compose locale e poi lancia il server di
...
È equivalente a eseguire `docker compose -f docker/docker-compose.community.local.yml up --build -d`
...
Password: il valore di `POSTGRES_PASSWORD` da `jinbocho-infrastructure-community-v1/.env`
...
`jinbocho-infrastructure-community-v1` include uno script che registra una famiglia di
